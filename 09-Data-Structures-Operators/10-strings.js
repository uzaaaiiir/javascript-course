...
for (const [i, flightItem] of flightArr.entries()) {
  let [title, from, to, time] = flightItem.split(';');

  // title
  title = title.replace('_', '');
  title = title.startsWith('Delayed') ? `🔴 ${title.replace('_', ' ')}` : title;

  const output = `${title} from ${from.slice(0, 3).toUpperCase()} to ${to
    .slice(0, 3)
